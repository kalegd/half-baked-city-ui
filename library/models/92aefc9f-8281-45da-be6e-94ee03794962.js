...
export default class DartGun {
    constructor(instance) {
        ...
        this._nextDartSpeed = instance['Dart Minimum Speed (m/s)'];
        this._gravity = new THREE.Vector3(0, -1 * instance['Gravity (m/s^2)'], 0);
    ...
    _createMeshes(instance) {
        ...
        gltfLoader.load(dartFilename,
            function (gltf) {
                scope._animations = gltf.animations;
                scope._dartScene = gltf.scene;
                scope._dartScene.scale.set(dartScale, dartScale, dartScale);
    ...
    _releaseTrigger() {
        if(this._nextDartDisabled) {
            ...
            this._createDart(this._instance);
        }
        this._triggerPressed = false;
    ...
    _updateCurve() {
        ...
        if(global.popGameController.mode == "NORMAL") {
            lifeSpan = this._instance['Dart Life Span (seconds)'];
        }
        for(let i = 1; i <= 10; i++) {
            let time = lifeSpan * i / 10;
            localPoints.push(new THREE.Vector2(time * initialSpeedX, time * initialSpeedY - (0.5 * this._instance['Gravity (m/s^2)'] * time * time)));
        }
    ...
    update(timeDelta) {
        ...
        for(let i in this._darts) {
            ...
            dart['velocity'].addScaledVector(this._gravity, timeDelta);
        ...
        if(this._darts.length > 0 && this._darts[0]['timeExisted'] > this._instance['Dart Life Span (seconds)']) {
