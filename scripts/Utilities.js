...
function createInputOfType(dataType, initialValue, name, instance) {
    if(dataType == "color") {
        ...
        return _createAudioInput(initialValue, name, instance);
...
function _createAudioInput(initialValue, name, instance) {
    let div = document.createElement("div");
    let input = document.createElement("input");
    let span = document.createElement("span");
    let b = document.createElement("b");
    ...
    if(!initialValue) {
        ...
        bText = document.createTextNode(dataStore.getAudioName(initialValue) + "\xa0\xa0");
    }
    b.append(bText);
    span.append(text);
    span.append(b);
    ...
    let dialog = _createAudioDialog(instance, name, div, input, b);
    div.append(dialog);
    return div;
}
function _createAudioDialog(instance, name, div, input, b) {
    let dialog = document.createElement("dialog");
    let dialogDiv = document.createElement("div");
    let h1 = document.createElement("h1");
    h1.append(document.createTextNode("Select Audio"));
    dialogDiv.append(h1);
    $(dialogDiv).addClass("editable-assets-list");
    dialogDiv.append(h1);
    dialog.append(dialogDiv);
    for(let audioId in dataStore.audios) {
        let audio = dataStore.audios[audioId];
        let a = document.createElement("a");
        ...
        a.addEventListener("click", function() {
            if(instance) {
                instance[name] = audioId;
                if(audioId != div.lastValidValue) {
                    dataStore.signalWebsiteUpdate();
                }
            }
            input.val = audioId;
            div.lastValidValue = audioId;
            b.innerHTML = audio.name + "\xa0\xa0";
            dialog.close();
        }, false);
        dialogDiv.append(a);
    }
...
function insertWrappedTextToCanvas(context, text, x, y, maxWidth, lineHeight) {
